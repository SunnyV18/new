codeunit 50113 ProcessItem_Designer_New
{
    ...
    //[TryFunction]
    procedure AssignCategories(CategorySizeStaging: Record CategorySizeStaging; Var ErrorText: Text): Boolean
    var
    ...
    begin
        ...
        IF CategorySizeStaging.Code = '' then begin
            ErrorText := 'Code must have a value in ' + Format(CategorySizeStaging.RecordId);
            exit(false);
        end;
        IF CategorySizeStaging."Sub Category Code" = '' then begin
            ErrorText := 'Sub Category Code must have a value in ' + Format(CategorySizeStaging.RecordId);
            exit(false);
        end;
    ...
    begin
        ...
        if CategorySizeStaging.Find('-') then
            repeat
                if not AssignCategories(CategorySizeStaging, ErrorText) then begin
                    cuFunctions.CreateErrorLog(8, ErrorText, CategorySizeStaging."Entry No", '', '', '');
    ...
    //[TryFunction]
    procedure CreateVendors(recDesignerStage: Record Aza_Designer): Boolean
    var
        // recDesignerStage: Record Aza_Designer;
    ...
    var
        ...
        ProcessItem_CreateItem: Codeunit ProcessItem_DesignerCreatItem;
        ProcessItem_ProcessImage: Codeunit ProcessItem_DesignerProceImage;
        ErrorMsg: Text;
    begin
        Clear(errorFound);
        ...
        if recItemStage.Find('-') then
            repeat
                ...
                if not recItemStage.Is_Outward then begin//020223 CITS_RS
                    Commit();
                    if ProcessItem_CreateItem.Run(recItemStage) then begin
                        Flag := true;

                        if recItem.get(txtBarcode) then
                            if not CU_Functions.UpdateGSTFields(recItem, ErrorMsg) then begin
                                cuFunctions.CreateErrorLog(1, ErrorMsg, recItemStage."Entry No.", format(recItemStage.designer_id), txtBarcode, '');
                                errorFound := true;
                        ...
                        ProcessItem_ProcessImage.SetBarcode(recItemStage.bar_code);
                        Commit();
                        if not ProcessItem_ProcessImage.Run then begin //170223 CITS_RS
